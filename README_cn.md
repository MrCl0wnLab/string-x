...
| `-pm` | 仅显示模块结果(省略shell输出) | `-pm` |
| `-pmc` | 分别显示链中每个模块的结果(不含shell输出) | `-pmc` |
| `-ps, -print-shell` | 即使启用了 `-pm`/`-pmc` 也显示(并保存)shell输出 | `-ps` |
| `-pf` | 仅显示函数结果 | `-pf` |
| `-of` | 将函数结果保存到文件 | `-of` |
...
- **`-pmc`**: 仅分别显示链中每个模块的结果(不含shell输出)
- **`-ps` / `-print-shell`**: 即使启用了 `-pm`/`-pmc` 也显示(并保存)shell输出
