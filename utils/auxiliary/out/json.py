...
class JSONOutput(BaseModule):
    ...
    def __init__(self):
        super().__init__()
        global _INITIALIZED, _COLLECTED_DATA
        ...
        self.meta = {
            ...
            'type': 'output',
            'example': './strx -l data.txt -st "echo {STRING}" -module "out:json" -pm'
        }
    ...
    def _save_all_data(self):
        ...
        with _JSON_LOCK:
            ...
            try:
                ...
                # Escreve diretamente no arquivo, sem tentar carregar o arquivo existente
                with open(file_path, 'w', encoding='utf-8') as f:
                    json.dump(entries, f, indent=2, ensure_ascii=False)
                
                self.log_debug(f"Successfully saved {len(entries)} items to {file_path}")
    ...
    def run(self):
        ...
        self.log_debug(f"Processing data: {data[:50]}{'...' if len(data) > 50 else ''}")
        ...
        with _JSON_LOCK:
            ...
            if count > 0 and count % batch_size == 0:
                self.log_debug(f"Batch size {batch_size} reached, triggering intermediary save")
    ...
    def _save_intermediary_batch(self):
        ...
        try:
            # Copia os dados atuais para evitar modificações durante o processamento
            ...
            # Escreve no arquivo sem tentar ler o existente para evitar corrupção
            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(entries, f, indent=2, ensure_ascii=False)
